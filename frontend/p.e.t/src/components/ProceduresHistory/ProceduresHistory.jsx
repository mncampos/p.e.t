...
export default function ProceduresHistory(){
    ...
    const fetchProcedures = async() => {
        const data = await fetch('/api/getCompletedProcedures')
       ...
       for(let i = 0; i < values.length; i++) {
        proceduresCards.push(
            <div className='procedureCard' key={i}>
                ...
                <h3 className='procedureCardText' key={i*9}>{`Start date : ${values[i].procedureStartDate}`}</h3>
                <h3 className='procedureCardText' key={i*11}>{`End date : ${values[i].procedureEndDate}`}</h3>
            </div>
        )
       }
